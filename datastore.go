...
func (d *Datastore) PutWithTTL(ctx context.Context, key ds.Key, value []byte, ttl time.Duration) error {
	...
	defer func() {
		txn.discard()
		if ts < 10 {
			fmt.Println("PutWithTTL Close")
		}
	}()

	if  ts < 10 {
		fmt.Println("PutWithTTL Start")
...
func (d *Datastore) SetTTL(ctx context.Context, key ds.Key, ttl time.Duration) error {
	...
	defer func() {
		...
		if txn.txn.ReadTs() < 10 {
			fmt.Println("SetTTL Close")
		}
	}()

	ts := txn.txn.ReadTs()
	if  ts < 10 {
		fmt.Println("SetTTL Start")
		fmt.Println("txn.ReadTs() = ", ts)
		fmt.Println("Key: ", key.String())
	}
...
func (d *Datastore) GetExpiration(ctx context.Context, key ds.Key) (time.Time, error) {
	...
	defer func() {
		txn.discard()
		if txn.txn.ReadTs() < 10 {
			fmt.Println("GetExpiration Close")
		}
	}()

	ts := txn.txn.ReadTs()
	if ts < 10 {
		fmt.Println("GetExpiration Start")
		fmt.Println("txn.ReadTs() = ", ts)
		fmt.Println("Key: ", key.String())
	}
...
func (d *Datastore) Get(ctx context.Context, key ds.Key) (value []byte, err error) {
	...
	defer func() {
		txn.discard()
		if txn.txn.ReadTs() < 10 {
			fmt.Println("Get Close ", txn.txn.ReadTs())
		}
	}()
...
func (d *Datastore) Has(ctx context.Context, key ds.Key) (bool, error) {
	...
	defer func() {
		txn.discard()
		if txn.txn.ReadTs() < 10 {
			fmt.Println("Haz Close ", txn.txn.ReadTs())
		}
	}()
	ts := txn.txn.ReadTs()
	if ts < 10 {
...
func (d *Datastore) GetSize(ctx context.Context, key ds.Key) (size int, err error) {
	...
	defer func() {
		txn.discard()
		if txn.txn.ReadTs() < 10 {
			fmt.Println("GetSize Close ", txn.txn.ReadTs())
		}
	}()
...
func (t *txn) query(q dsq.Query) (dsq.Results, error) {
	...
	qrb.Process.Go(func(worker goprocess.Process) {
		...
		if t.implicit {
			fmt.Println("Query Start: ", q.String())
			defer func() {
				fmt.Println("Query Close: ", q.String())
				t.discard()
			}()
		}
